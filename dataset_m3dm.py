...
class Dataset3dad_train(Dataset):
    def __init__(self, dataset_dir, cls_name, num_points, if_norm=False, if_cut=False):
...
class Dataset3dad_test(Dataset):
    def __init__(self, dataset_dir, cls_name, num_points, if_norm=False, if_cut=False):
        ...
        self.test_sample_list = [s for s in test_sample_list if 'temp' not in s]
        cut_list = [s for s in self.test_sample_list if 'cut' in s or 'copy' in s]
        # if if_cut:
