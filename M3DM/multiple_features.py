import torch
from M3DM.features import Features,Features1
...
import math
import os
from utils.visualization import save_anomalymap_points
...
class PointFeatures(Features1):

    def add_sample_to_mem_bank(self, sample):
        organized_pc = sample[1]
        # organized_pc = organized_pc.transpose(1,2).contiguous().view(1, 3,32, 32)
        # organized_pc_np = organized_pc.squeeze().permute(1, 2, 0).numpy()
        # unorganized_pc = organized_pc_to_unorganized_pc(organized_pc=organized_pc_np)
        unorganized_pc = organized_pc.squeeze().numpy()
        nonzero_indices = np.nonzero(np.all(unorganized_pc != 0, axis=1))[0]
        # print("nonzero_indices",nonzero_indices.shape)
        # print("nonzero_indices",nonzero_indices.shape[0])
        
        unorganized_pc_no_zeros = torch.tensor(unorganized_pc[nonzero_indices, :]).unsqueeze(dim=0).permute(0, 2, 1)
        xyz_feature_maps, center, neighbor_idx, center_idx = self(unorganized_pc_no_zeros.contiguous())
        # print("center_idx:",center_idx.shape)
        # print("xyz_feature_maps",xyz_feature_maps.shape)
        xyz_patch = torch.cat(xyz_feature_maps, 1)
        xyz_patch = xyz_patch.squeeze().transpose(0,1)
        # print("xyz_patch",xyz_patch.shape)

        # xyz_patch_full = torch.zeros((1, interpolated_pc.shape[1], nonzero_indices.shape[0]), dtype=xyz_patch.dtype)
    ...
    def predict(self, sample, mask, label,pcd_path,save_path_full,save_path):
        organized_pc = sample[1]
        # organized_pc_np = organized_pc.squeeze().permute(1, 2, 0).numpy()
        # unorganized_pc = organized_pc_to_unorganized_pc(organized_pc=organized_pc_np)
        unorganized_pc = organized_pc.squeeze().numpy()
        ...
        unorganized_pc_no_zeros = torch.tensor(unorganized_pc[nonzero_indices, :]).unsqueeze(dim=0).permute(0, 2, 1)
        # print("unorganized_pc_no_zeros",unorganized_pc_no_zeros.shape)
        xyz_feature_maps, center, neighbor_idx, center_idx = self(unorganized_pc_no_zeros.contiguous())
        ...
        mask = mask[:,mask_idx]
        # print("mask.shape",mask.shape)
        xyz_sampled = unorganized_pc[mask_idx.cpu(),:]
        ...
        s_map = self.compute_s_s_map(xyz_patch, mask, label, center, neighbor_idx, nonzero_indices, unorganized_pc_no_zeros.contiguous(), center_idx)
        # print("s_map.shape",s_map.shape)
        # print("xyz_patch:",xyz_patch.shape)
        # print("mask:",mask.shape)
        anomaly_map = fill_missing_values(xyz_sampled,s_map.cpu().numpy(),unorganized_pc)
        # print("anomaly_map:",anomaly_map.shape)
        # print("pcd_path:",pcd_path,type(pcd_path))
        '''
        #save visualization and anomaly map numpy
        pcd_path = ''.join(pcd_path)
        # print("pcd_path:",pcd_path,type(pcd_path))
        path_name = pcd_path.split('/')[-1]
        path_class = pcd_path.split('/')[-3]
        # print("path_name",path_name)
        # print("path_class",path_class)
        if not os.path.exists(os.path.join(save_path_full, path_class)):
            os.makedirs(os.path.join(save_path_full, path_class))
        if not os.path.exists(os.path.join(save_path, path_class)):
            os.makedirs(os.path.join(save_path, path_class))

        pcd_saveresult_path_full = os.path.join(save_path_full, path_class,path_name)
        pcd_saveresult_path = os.path.join(save_path, path_class,path_name)
        # print("pcd_saveresult_path",pcd_saveresult_path)
        save_anomalymap_points(unorganized_pc,anomaly_map,pcd_saveresult_path_full)
        save_anomalymap_points(xyz_sampled,s_map.cpu().numpy(),pcd_saveresult_path)
        save_pcd_path = os.path.join(save_path_full,path_class,path_name.replace('pcd','npy'))
        np.save(save_pcd_path,anomaly_map)
        '''

    def run_coreset(self,class_name):
        ...
        torch.save(self.patch_lib, "/data1/chenruitao/M3DM/M3DM/patch_lib/"+class_name+"_patch_lib.pth")


    def compute_s_s_map(self, patch, mask, label, center, neighbour_idx, nonzero_indices, xyz, center_idx, nonzero_patch_indices = None):
        '''
        center: point group center position
        neighbour_idx: each group point index
        nonzero_indices: point indices of original point clouds
        ...
        self.pixel_labels.extend(mask.flatten().numpy())
        self.predictions.append(s_map.detach().cpu().squeeze().numpy())
        # print("s_map.detach().cpu().squeeze().numpy()",s_map.detach().cpu().squeeze().numpy().shape)
        self.gts.append(mask.detach().cpu().squeeze().numpy())
        # print("mask.detach().cpu().squeeze().numpy()",mask.detach().cpu().squeeze().numpy().shape)
        return s_map
